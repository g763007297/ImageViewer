...
@interface DemoViewController ()<GQImagePreviewerDelegate, GQImagePreviewerDataSource>{
    ...
    NSMutableArray *_textArray;//文字数组
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    _demoView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.view.frame)/2-100, 64, 200, 300)];
...
- (void)showManualManagerImageViewer:(id)sender {
    ...
    .configureChain(^(GQImageViewrConfigure *configure) {
        ...
        configure.launchFromView = self->_demoView;
        [configure setNeedTapAutoHiddenTopBottomView:YES];//设置是否需要自动隐藏顶部和底部视图
    })
    .dataSouceArrayChain(_imageArray,_textArray)//如果仅需要图片浏览就只需要传图片即可，无需传文字数组
...
- (void)showAutoManagerImageViewer:(id)sender{
    //    //基本调用
    //    [[GQImageViewer sharedInstance] set_imageArray:_imageArray _textArray:nil];//这是数据源
...
- (void)addLocalImage {
    ...
    NSMutableAttributedString * mAttribute = [[NSMutableAttributedString alloc] initWithString:@"1.百度百科是百度公司推出的一部内容开放、自由的网络百科全书平台。其测试版于2006年4月20日上线，正式版在2008年4月21日发布，截至2016年4月，百度百科已经收录了超过1300多万的词条，参与词条编辑的网友超过580万人，几乎涵盖了所有已知的知识领域。"];
    ...
    [mAttribute addAttribute:NSFontAttributeName
                       ...
                       range:NSMakeRange(0, [mAttribute string].length)];
    
    [_textArray addObjectsFromArray:@[
                                     mAttribute,
...
- (void)addWebpImage {
    ...
    [_textArray addObjectsFromArray:@[@"11.webp图片测试1",
                                     @"12.webp图片测试"]];
}
